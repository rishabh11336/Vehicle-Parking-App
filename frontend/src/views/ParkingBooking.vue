          ...
          <div class="card p-4">
            ...
            <form @submit.prevent="handleBooking">
              ...
              <div v-if="bookingDetails.type === 'hourly'">
                <div class="row">
                  <div class="col-md-4 mb-3"><label for="bookingDate" class="form-label fw-semibold">Date</label><input type="date" class="form-control" id="bookingDate" v-model="bookingDetails.date" required></div>
              ...
              <div v-if="bookingDetails.type === 'monthly'">
                <div class="row">
                  <div class="col-md-6 mb-3"><label for="startDate" class="form-label fw-semibold">Start Date</label><input type="date" class="form-control" id="startDate" v-model="bookingDetails.date" required></div>
                  <div class="col-md-6 mb-3"><label for="endDate" class="form-label fw-semibold">End Date</label><input type="date" class="form-control" id="endDate" v-model="bookingDetails.endDate" required></div>
              ...
              <div v-if="bookingDetails.type === 'permanent'">
                <div class="row">
                  <div class="col-md-6 mb-3"><label for="permStartDate" class="form-label fw-semibold">Start Date</label><input type="date" class="form-control" id="permStartDate" v-model="bookingDetails.date" required></div>
                  <div class="col-md-6 mb-3"><label for="permEndDate" class="form-label fw-semibold">End Date</label><input type="date" class="form-control" id="permEndDate" v-model="bookingDetails.endDate" required></div>
                </div>
              </div>
...
export default {
  ...
  computed: {
    bookingCalculation() {
        ...
        const { startTime, endTime, date, endDate } = this.bookingDetails;

        if (type === 'permanent') {
            const startD = new Date(date);
            const endD = new Date(endDate);
            if (endD < startD) return { totalDays: 0, hoursPerDay: 0, appliedRate: 0, totalCost: 0, rateType: 'Invalid' };
            const totalDays = ((endD - startD) / (1000 * 60 * 60 * 24)) + 1;
            const appliedRate = (totalDays >= 2) ? (this.selectedLot.price_per_hour || 80) * 0.8 : (this.selectedLot.price_per_hour || 80);
            const rateType = (totalDays >= 2) ? 'Permanent (20% Off)' : 'Standard';
            const totalCost = totalDays * 24 * appliedRate;
            return { totalDays, hoursPerDay: 24, appliedRate, totalCost, rateType };
        }

        const startT = new Date(`1970-01-01T${startTime}`);
        const endT = new Date(`1970-01-01T${endTime}`);
        const hoursPerDay = (endT.getTime() - startT.getTime()) / (1000 * 60 * 60);

        if (hoursPerDay <= 0) return { totalDays: 0, hoursPerDay: 0, appliedRate: 0, totalCost: 0, rateType: 'Invalid' };
        ...
        return { totalDays: Math.round(totalDays), hoursPerDay, appliedRate, totalCost, rateType };
    },
    appliedRateText() {
    ...
    parkingDurationText() {
        const { totalDays, hoursPerDay } = this.bookingCalculation;
        if (!totalDays) return 'N/A';
        if (this.bookingDetails.type === 'permanent') return `${totalDays} Days (24/7)`;
        if (this.bookingDetails.type === 'hourly') return `${hoursPerDay.toFixed(1)} Hours`;
        return `${totalDays} Days (${hoursPerDay.toFixed(1)} hrs/day)`;
    },
    totalCost() {
    ...
    isBookingValid() {
        const { totalDays, hoursPerDay } = this.bookingCalculation;
        const durationValid = totalDays > 0 && (hoursPerDay > 0 || this.bookingDetails.type === 'permanent');
  ...
  methods: {
    async fetchLotDetails(lotId) {
        ...
        try {
            ...
            if (lot) {
                lot.pricePerHour = lot.price; 
                this.selectedLot = lot;
                this.selectedLot.imageUrl = `https://placehold.co/600x400/A7C7E7/FFFFFF?text=${lot.name.replace(' ', '+')}`;
            } else {
                this.errorMessage = "Parking lot not found.";
            }
        } catch (error) {
            console.error("Failed to fetch lot details:", error);
            this.errorMessage = "Could not load parking lot details.";
        } finally {
    ...
    async handleBooking() {
      ...
      try {
        const payload = {
          ...
          start_time: this.bookingDetails.startTime,
          end_time: this.bookingDetails.endTime
        };
